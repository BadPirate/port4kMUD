...
{ int subcmd = 0, counter = 0;
  ...
  { /*. Translate what the command means .*/
    ...
    { case'M':
        sprintf(buf2, "%sLoad %s [%s%d%s], Max : %d Perc : %d",
                MYCMD.if_flag ? " then " : "",
        ...
        break;
      case'G':
        sprintf(buf2, "%sGive it %s [%s%d%s], Max : %d Perc : %d",
		MYCMD.if_flag ? " then " : "",
      ...
      case'O':
        ...
        break;
      case'E':
        sprintf(buf2, "%sEquip with %s [%s%d%s], %s, Max : %d Perc : %d",
		MYCMD.if_flag ? " then " : "",
		obj_proto[MYCMD.arg1].short_description,
                cyn, obj_index[MYCMD.arg1].virtual, yel,
        ...
        break;
      case'P':
        sprintf(buf2, "%sPut %s [%s%d%s] in %s [%s%d%s], Max : %d Perc : %d",
		MYCMD.if_flag ? " then " : "",
		obj_proto[MYCMD.arg1].short_description,
                cyn, obj_index[MYCMD.arg1].virtual, yel,
...
void zedit_disp_arg4(struct descriptor_data *d)
{
  switch(OLC_CMD(d).command)
  {
    ...
    case 'E':
    case 'P':
      send_to_char("Enter percent chance of loading: ", d->character);
      break;
    default:
      log("Error in display_arg4");
      break;
  }
    ...
    switch(OLC_CMD(d).command)
    {
      ...
      case 'P':
        if (atoi(arg) >= 0 && atoi(arg) <= 100)
        {
          ...
          send_to_char("1 to 100 please : ", d->character);
        break;
      default:
        log("Bummer dude, in arg4");
        cleanup_olc(d, CLEANUP_ALL);
        break;
    }            
